...
LOCAL void HTTPConfig_TCPServerRecieveCallback(void *arg, char *pusrdata, unsigned short length) {
	...
	if (os_strcmp(pusrdata, "GET / HTTP/1.1") == 0) {
		char *html = NULL;
		html = (char *) os_malloc(1024 * sizeof(char));
		os_sprintf(html, "<!DOCTYPE html>\
			...
			<body>\
				<object data=\"https://esp8266.valkenb.org/config/v1/\"></object>\
		</html>");
		HTTPConfig_TCPServerSendResponse(pespconn, 200, html);
	}
	else if (os_strcmp(pusrdata, "GET /data HTTP/1.1") == 0) {
		CFG_Load();
